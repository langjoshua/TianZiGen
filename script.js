...
import { svg2pdf } from "https://esm.sh/svg2pdf.js";
...
function getFinalChars() {
  ...
  const example = match ? match[1] : "我喜欢写字";
...
function getLayout() {
  ...
  return { pageWidth, pageHeight, marginTop, gap, cols, cellSize, offsetX, rowsPerPage };
}
...
function buildPagesNormal(chars, rowsPerPage) {
  if (chars.length <= rowsPerPage) {
    return [Array.from({ length: rowsPerPage }, (_, i) => chars[i % chars.length])];
  }
  const pages = [];
...
function buildNormalPage(rows, dataMap, layout) {
  const { pageWidth, pageHeight, marginTop, gap, cols, cellSize, offsetX } = layout;
  ...
  rows.forEach((char, row) => {
    ...
    const py = showPinyin
      ? pinyin(char, { toneType: "num", type: "array" })[0].toLowerCase()
      : "";
...
async function generate(chars) {
  ...
  if (raw.length && !rawChars.length) {
    message.textContent = "No Chinese characters detected";
  } else if (unsupported.length) {
    message.textContent = `Unsupported: ${unsupported.join(" ")}`;
  } else {
    message.textContent = "";
  }
  ...
  const doc = new jsPDF({ unit: "px", format: [layout.pageWidth, layout.pageHeight] });
...
document.getElementById("share-btn")?.addEventListener("click", async () => {
  ...
  const file = new File([blob], "worksheet.pdf", { type: "application/pdf" });

  if (navigator.share && navigator.canShare?.({ files: [file] })) {
    try {
      await navigator.share({ title: "Chinese Worksheet", files: [file] });
    } catch (e) {
      console.error(e);
    }
  } else {
